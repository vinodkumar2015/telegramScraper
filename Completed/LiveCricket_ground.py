...
async def main():
    ...
    channel_username = 'GROUND_LINE_ORIGINAL'
    ...
    while time.time() < end_time:
        ...
        message = await next(client.iter_messages(channel_username, limit=recent_message_limit, reverse=False), None)
            ...
            if message.text and (latest_message_id is None or message.id > latest_message_id):
                ...
                if "https" not in message.text and "http" not in message.text and "**.**" not in message.text:
                    ...
                    if "**RUKA✔️✔️**" in message.text:
                        await client.send_message('charlie_live_cricket', f"\n wait for a moment")
